...
namespace Toki {
    ...
    VkImage VulkanTexture::createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkDeviceMemory& imageMemory) {
        ...
        TK_ASSERT(vkAllocateMemory(device, &allocInfo, nullptr, &imageMemory) == VK_SUCCESS);
    ...
    VkImageView VulkanTexture::createImageView(const VkImage& image, VkFormat format, VkImageAspectFlags aspectFlags) {
