...
void GameOfLifeView::on_destroy() {
    toki::Renderer& renderer = get_renderer();
...
void GameOfLifeView::on_update(float delta_time) {
    ...
    if (m_dirty) {
        update_cell_data();
        if (m_cellCount > 0) {
            get_renderer().set_buffer_data(m_tileInstanceBufferHandle, sizeof(Cell) * m_cellCount, m_cellInstanceData.data());
        }
    }
...
void GameOfLifeView::on_render() {
    toki::Renderer& api = get_renderer();
