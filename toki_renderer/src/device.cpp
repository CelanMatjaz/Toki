...
TkError create_instance(RendererState* state) {
    ...
    TK_ASSERT_VK_RESULT(
        vkCreateInstance(&instance_create_info, state->allocation_callbacks, &state->instance),
...
TkError create_device(RendererState* state) {
    Scope<GLFWwindow*, nullptr> temp_window(
        glfwCreateWindow(1, 1, "", nullptr, nullptr), [](GLFWwindow* w) { glfwDestroyWindow(w); });

    Scope<VkSurfaceKHR, VK_NULL_HANDLE> temp_surface(VK_NULL_HANDLE, [&](VkSurfaceKHR s) {
        vkDestroySurfaceKHR(state->instance, s, state->allocation_callbacks);
    });
    create_surface(state, temp_window.value(), &temp_surface.ref());
    ...
    vkEnumeratePhysicalDevices(state->instance, &physical_device_count, physical_devices.data());
    ...
    TK_ASSERT_VK_RESULT(
        vkCreateDevice(
            physical_device, &device_create_info, state->allocation_callbacks, &state->device),
